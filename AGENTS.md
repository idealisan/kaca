...
- **文字输入合并**：没有被其他动作打断的连续按键，合并为「一次文字输入」动作（`STEP_EVENT_TYPE`），报告里显示为「用户输入了文字」+ 原文块；停顿 >3 秒则把当前段定稿，并在下一段标注「停顿 N 秒后继续」。非文字键（Esc/方向/F 键/纯修饰键等，`key_text` 为空）不并入，按普通按键记录，并先结束当前输入段。输入法场景只能捕获按键本身、无法保证拿到最终合成文字（已知限制）。
